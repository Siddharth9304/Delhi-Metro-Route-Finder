...
// DFS function to find the route
function findRoute(graph, start, destination) {
    let path = [];
    ...
    function dfs(current) {
        if (current === destination) {
            route.push(current);
            routeLines.push(stationColors.get(current));
            return true;
        }
        visited.add(current);
    ...
    // Start DFS from the starting station
    if (dfs(start)) {
        // path.push(`Route from ${start} to ${destination}:`);
        ...
        for (let i = route.length - 1; i >= 0; --i) {
            ...
            if (!firstStation && line !== routeLines[i + 1]) {
                if(i==0 || routeLines[i-1]!==routeLines[i+1]){
                    if(flag==true){
                        if(isInterchangeStation(station)){
                            ...
                            if(i!==route.length-2 && flag1===true)
                                path.push("Interchange");
                            else
                                flag1 = true;
